...
use core::{fmt::{self, Write}, ops::{Deref, DerefMut}};
use register::{register_bitfields, register_structs,  mmio::{ReadOnly, ReadWrite, WriteOnly}};
use crate::asm::block_until;

// see page 656 of the rk3399 technical reference manual
register_bitfields! {
    u32,
    /// Shared (overlapping):
    ...
    ///
    /// Shared (overlapping) UART_DLH Divisor Latch (High)
    IER [
        // DLH fields
    ...
    ///
    /// Shared (overlapping) UART_FCR FIFO Control Register
    IIR [
        // IIR fields
    ...
    SRBR [
        ...
        /// mode or the serial infrared input (sir_in) in infrared
        /// mode. The data in this register is valid only if the Data
        /// Ready (DR) bit in the Line status Register (LSR) is
        /// set. If FIFOs are disabled (FCR[0] set to zero), the data
        /// in the RBR must be read before the next data arrives,
        /// otherwise it is overwritten, resulting in an overrun
        /// error. If FIFOs are enabled (FCR[0] set to one), this
        /// register accesses the head of the receive FIFO. If the
        /// receive FIFO is full and this register is not read before
        /// the next data character arrives, then the data already in
        /// the FIFO are preserved, but any incoming data is lost. An
        /// overrun error also occurs.
        shadow_rbr OFFSET(0) NUMBITS(8) []
    ],
    /// Shadow Transmit Holding Register
    STHR [
        /// This is a shadow register for the THR.
        shadow_thr OFFSET(0) NUMBITS(8) []
    ],

    /// FIFO Access Register
    FAR [
        /// This register is use to enable a FIFO access mode for
        /// testing, so that the receive FIFO can be written by the
        /// master and the transmit FIFO can be read by the master
        /// when FIFOs are implemented and enabled. When FIFOs are not
        /// enabled it allows the RBR to be written by the master and
        /// the THR to be read by the master.
        fifo_access_test_en OFFSET(0) NUMBITS(1) [
            Disabled = 0,
            Enabled = 1
        ]
    ],
    ...
    /// Transmit FIFO Read
    TFR [
        /// Transmit FIFO Read. These bits are only valid when FIFO
        /// access mode is enabled (FAR[0] is set to one).When FIFOs
        /// are implemented and enabled, reading this register gives
        /// the data at the top of the transmit FIFO. Each consecutive
        /// read pops the transmit FIFO and gives the next data value
        /// that is currently at the top of the FIFO.
        trans_fifo_read OFFSET(0) NUMBITS(8) []
    ],

    /// Receive FIFO Write
    RFW [
        /// Receive FIFO Framing Error. These bits are only valid when
        /// FIFO access mode is enabled (FAR[0] is set to one).
        receive_fifo_framing_error OFFSET(9) NUMBITS(1) [],
        /// Receive FIFO Parity Error. These bits are only valid when
        /// FIFO access mode is enabled (FAR[0] is set to one).
        receive_fifo_parity_error OFFSET(8) NUMBITS(1) [],
        /// Receive FIFO Write Data. These bits are only valid when
        /// FIFO access mode is enabled (FAR[0] is set to one). When
        /// FIFOs are enabled, the data that is written to the RFWD is
        /// pushed into the receive FIFO. Each consecutive write
        /// pushes the new data to the next write location in the
        /// receive FIFO. When FIFOs not enabled, the data that is
        /// written to the RFWD is pushed into the RBR.
        receive_fifo_write OFFSET(0) NUMBITS(8) []
    ],

    /// UART Status Register
    USR [
        /// Receive FIFO Full. This is used to indicate that the
        /// receive FIFO is completely full
        receive_fifo_full OFFSET(4) NUMBITS(1) [
            NonFull = 0,
            Full = 1
        ],
        /// Receive FIFO Not Empty. This is used to indicate that the
        /// receive FIFO contains one or more entries.
        receive_fifo_not_empty OFFSET(3) NUMBITS(1) [
            Empty = 0,
            NonEmpty = 1
        ],
        /// Transmit FIFO Empty. This is used to indicate that the
        /// transmit FIFO is completely empty.
        trans_fifo_empty OFFSET(2) NUMBITS(1) [
            NonEmpty = 0,
            Empty = 1
        ],
        /// Transmit FIFO Not Full. This is used to indicate that the
        /// transmit FIFO in not full.
        trans_fifo_not_full OFFSET(1) NUMBITS(1) [
            Full = 0,
            NonFull = 1
        ],
        /// UART Busy. This is indicates that a serial transfer is in
        /// progress, when cleared indicates that the UART is idle or
        /// inactive.
    ...
    TFL [
        ...
        /// transmit FIFO.
        trans_fifo_level OFFSET(0) NUMBITS(4) []
    ],
    ...
    RFL [
        /// This is indicates the number of data entries in the
        /// receive FIFO.
        receive_fifo_level OFFSET(0) NUMBITS(4) []
    ],

    /// Software Reset Register
    SRR [
        /// XMIT FIFO Reset. This is a shadow register for the XMIT
        /// FIFO Reset bit (FCR[2]).
        xmit_fifo_reset OFFSET(2) NUMBITS(1) [],
        /// RCVR FIFO Reset. This is a shadow register for the RCVR
        /// FIFO Reset bit (FCR[1]).
        rcvr_fifo_reset OFFSET(1) NUMBITS(1) [],
        ...
        uart_reset OFFSET(0) NUMBITS(1) []
    ],

    /// Shadow Request to Send
    SRTS [
        /// Shadow Request to Send. This is a shadow register for the
        /// RTS bit (MCR[1]), this can be used to remove the burden of
        /// having to performing a read-modify-write on the MCR.
        shadow_req_to_send OFFSET(0) NUMBITS(1) []
    ],

    /// Shadow Break Control
    SBCR [
        /// Shadow Break Control Bit. This is a shadow register for
        /// the Break bit (LCR[6]), this can be used to remove the
        /// burden of having to performing a read modify write on the
        /// LCR.
        shadow_break_ctrl OFFSET(0) NUMBITS(1) []
    ],

    /// Shadow DMA Mode
    SDMAM [
        /// Shadow DMA Mode. This is a shadow register for the DMA
        /// mode bit (FCR[3]).
        shadow_dma_mode OFFSET(0) NUMBITS(1) []
    ],
    ...
    SFE [
        ...
        /// enable bit (FCR[0]).
        shadow_fifo_en OFFSET(0) NUMBITS(1) [
            Disabled = 0,
            Enabled = 1
        ]
    ],
    ...
    /// Shadow RCVR Trigger
    SRT [
        /// Shadow RCVR Trigger. This is a shadow register for the
    ...
    STET [
        /// Shadow TX Empty Trigger. This is a shadow register for the
        /// TX empty trigger bits (FCR[5:4]).
    ...
    DMASA [
        /// This register is use to perform a DMA software acknowledge
        /// if a transfer needs to be terminated due to an error
        /// condition.
        dma_software_ack OFFSET(0) NUMBITS(1) []
    ]
}
...
register_structs! {
    RegisterBlock {
        ...
        (0x18 => msr: ReadOnly<u32, MSR::Register>),
        (0x1c => scr: ReadOnly<u32, SCR::Register>),
        (0x30 => srbr: ReadOnly<u32, SRBR::Register>),
        (0x6c => sthr: WriteOnly<u32, STHR::Register>),
        (0x70 => far: ReadWrite<u32, FAR::Register>),
        (0x74 => tfr: ReadOnly<u32, TFR::Register>),
        (0x78 => rfw: WriteOnly<u32, RFW::Register>),
        (0x7c => usr: ReadOnly<u32, USR::Register>),
        (0x80 => tfl: ReadOnly<u32, TFL::Register>),
        (0x84 => rfl: ReadOnly<u32, RFL::Register>),
        (0x88 => srr: WriteOnly<u32, SRR::Register>),
        (0x8c => srts: ReadWrite<u32, SRTS::Register>),
        (0x90 => sbcr: ReadWrite<u32, SBCR::Register>),
        (0x94 => sdmam: ReadWrite<u32, SDMAM::Register>),
        (0x98 => sfe: ReadWrite<u32, SFE::Register>),
        (0x9c => srt: ReadWrite<u32, SRT::Register>),
        (0xa0 => stet: ReadWrite<u32, STET::Register>),
        (0xa4 => htx: ReadWrite<u32, HTX::Register>),
        (0xa8 => dmasa: WriteOnly<u32, DMASA::Register>),
        (0x100 => @END),
    }
}
...
impl Uart {
    /// invariants: `base` must point to one of the board's UARTs; no
    /// `Uart` instance may already exist with the same `base`.
    const unsafe fn new(base: *mut RegisterBlock) -> Self { Uart {
        base
    } }
    fn can_write(&mut self) -> bool {
        self.lsr.matches_all(LSR::trans_empty::Empty)
    }
    fn write_byte(&mut self, b: u8) {
        block_until(|| self.can_write(), 1);
        self.data.set(b as u32);
    }
}
...
static UART2: Mutex<Uart> = Mutex::new(unsafe {
    Uart::new(0xff1a_0000 as *mut RegisterBlock)
});
